...
def getting_sql_query(ti,table,player_id):
    df=ti.xcom_pull(task_ids='scraping_new_players',key=f'{player_id}_{table}')
...
def inserting_players_career_path(ti):
    global postgres_connection
    result=ti.xcom_pull(task_ids='read_db')
    players_ids=[a[0] for a in result]
    for player_id in players_ids:
...
def inserting_players_stats_career(ti):
    global postgres_connection
    result=ti.xcom_pull(task_ids='read_db')
    players_ids=[a[0] for a in result]
    for player_id in players_ids:
        query, params=getting_sql_query(ti,'players_stats_career',player_id)
        print(query, params)
        SQLExecuteQueryOperator(
            task_id="upload_to_postgres",
            conn_id=postgres_connection,
