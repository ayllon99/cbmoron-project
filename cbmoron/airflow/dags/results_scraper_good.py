...
def getting_sql_query(ti,a):
    df=ti.xcom_pull(task_ids='scraping_results',key=f'matchday_{a}_df_results')
...
def results_scraper(ti):
        ...
        for a in aa:
                ...
                ti.xcom_push(key=f'matchday_{a}_match_links',value=match_links)
                ti.xcom_push(key=f'matchday_{a}_df_results',value=results)
        driver.quit()
