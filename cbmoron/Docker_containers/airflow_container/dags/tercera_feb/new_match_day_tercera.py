...
def new_time_column(shooting_df,data):
    ...
    time_shoot=datetime.strptime(time_list[0],"%M:%S").time()
    shooting_df['shoot_time']=shooting_df['shoot_time'].apply(lambda x: datetime.strptime("00:00","%M:%S").time() if x=='Nan' else x)
    return shooting_df
...
def match_day(ti):
    ...
    for group in groups:
        ...
        for match_day in match_days:
            links=ti.xcom_pull(task_ids='scraping_results',key=f'group_{group}_matchday_{match_day}_match_links')
            urls.extend(links)
