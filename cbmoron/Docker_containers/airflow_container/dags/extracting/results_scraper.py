...
def getting_sql_query(ti,match_day,stage_id):
    df=ti.xcom_pull(task_ids='scraping_results',key=f'stage_id_{stage_id}_matchday_{match_day}_df_results')
...
def inserting_to_postgres(ti,**op_kwargs):
        ...
        for stage_id in stages_ids:
                match_days=ti.xcom_pull(task_ids='evaluate_matchdays',key=f'{stage_id}_match_days') 
                print(match_days)
...
def results_scraper(ti,**op_kwargs):
        ...
        for stage_id in stages_ids:
                ...
                for match_day in match_days:
                        ...
                        ti.xcom_push(key=f'stage_id_{stage_id}_matchday_{match_day}_match_links',value=match_links)
                        ti.xcom_push(key=f'stage_id_{stage_id}_matchday_{match_day}_df_results',value=results)
        driver.quit()
