...
def new_results(ti):
    ...
    print(os.path.join(os.path.dirname(__file__), 'primera_feb/dates.txt'))
    file_path = os.path.join(os.path.dirname(__file__), 'primera_feb/dates.txt')
    ...
    driver.quit()
    if len(match_days)>0:
        ti.xcom_push(key='match_days',value=match_days)
        return True
    else:
        return False
