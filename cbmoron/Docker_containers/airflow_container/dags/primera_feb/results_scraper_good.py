...
def getting_sql_query(ti,match_day):
    df=ti.xcom_pull(task_ids='scraping_results',key=f'matchday_{match_day}_df_results')
...
def inserting_to_postgres(ti):
    ...
    match_days=ti.xcom_pull(task_ids='evaluate_matchdays',key='match_days')
    for match_day in match_days:
        query, params=getting_sql_query(ti,a=match_day)
        print(query, params)
...
def results_scraper(ti):
        ...
        for match_day in match_days:
                ...
                ti.xcom_push(key=f'matchday_{match_day}_match_links',value=match_links)
                ti.xcom_push(key=f'matchday_{match_day}_df_results',value=results)
        driver.quit()
