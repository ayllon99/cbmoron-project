...
file_path_primera = os.path.join(os.path.dirname(__file__), 'dates_files/dates_primera.txt')
file_path_segunda = os.path.join(os.path.dirname(__file__), 'dates_files/dates_segunda.txt')
file_path_tercera = os.path.join(os.path.dirname(__file__), 'dates_files/dates_tercera.txt')


def trigger_evaluator_primera(ti):
    result_primera=ti.xcom_pull(task_ids='evaluate_and_trigger_primera',key='trigger_evaluator')
    if result_primera==False:
...
def trigger_evaluator_segunda(ti):
    result_segunda=ti.xcom_pull(task_ids='evaluate_and_trigger_segunda',key='trigger_evaluator')
    if result_segunda==False:
        print('Result == False here the exception comes')
        raise AirflowFailException('---------------NO NEW MATCHES TO SCRAPE---------------')
    print('New matches to scrape, triggering next DAG...')
...
def trigger_evaluator_tercera(ti):
    result_tercera=ti.xcom_pull(task_ids='evaluate_and_trigger_tercera',key='trigger_evaluator')
    if result_tercera==False:
        print('Result == False here the exception comes')
        raise AirflowFailException('---------------NO NEW MATCHES TO SCRAPE---------------')
    print('New matches to scrape, triggering next DAG...')
...
with DAG(
    ...
    evaluate_and_trigger_primera = PythonOperator(
        ...
        op_kwargs={'url':url_primera,'file_path':file_path_primera}
