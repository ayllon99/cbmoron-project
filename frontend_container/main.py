...
list_of_players_dict = list_of_players_df.set_index('player_name')\
    .to_dict()['player_id']
list_of_players_total = [a.lower() for a in
                         list_of_players_df['player_name'].to_list()]
...
stats_columns = [x for x in my_list if x not in
...
def submit_stats(state):
    ...
    print(season, n_matches, min_avg, points_avg, twos_in_avg, twos_tried_avg,
          twos_perc, threes_in_avg, threes_tried_avg, threes_perc,
          field_goals_in_avg, field_goals_tried_avg, field_goals_perc,
          free_throws_in_avg, free_throws_tried_avg, free_throws_perc,
          offensive_rebounds_avg, deffensive_rebounds_avg, total_rebounds_avg,
          assists_avg, turnovers_avg, blocks_favor_avg, blocks_against_avg,
          dunks_avg, personal_fouls_avg, fouls_received_avg, efficiency_avg)
...
with tgb.Page() as player_analysis:
    ...
    # tgb.text(' ',mode='pre')
    # tgb.text('Use the searcher bar to type the name of the player and
    # select the player in the left menu',mode='pre',class_name='text-center')
    """with tgb.layout(1):
        with tgb.layout(class_name='d-inline-block m-auto'):
            with tgb.part(class_name=
            'd-inline-block m-auto d-flex justify-content-center'):
                tgb.input('{value}',type='search',label='Player name',
                          class_name='d-inline',on_change=name_scraper,
                          change_delay=10)
            with tgb.part(class_name=
            'd-inline-block m-auto d-flex justify-content-center'):
    ...
    with tgb.layout("1 1 1 1"):
        with tgb.part():
            tgb.selector(value='{league_selected}',
                         lov='{leagues_list}',
                         dropdown=True,
                         multiple=False,
                         mode='selector',
        ...
        with tgb.part():
            tgb.selector(value='{season_selected}',
                         lov='{seasons_list}',
                         dropdown=True,
                         multiple=False,
                         mode='selector',
        ...
        with tgb.part():
            tgb.selector(value='{team_selected}',
                         lov='{teams_list}',
                         dropdown=True,
                         multiple=False,
                         mode='selector',
        ...
        with tgb.part():
            tgb.selector(value='{player_selected}',
                         lov='{players_list}',
                         dropdown=True,
                         multiple=False,
                         mode='selector',
